...
const CACHE_NAME = 'javadian-operations-shell-v1.2.0';
  ...
  event.waitUntil(
    caches.open(CACHE_NAME).then((cache) => {
      return cache.addAll(PRECACHE_ASSETS).catch((err) => {
        console.warn('[ServiceWorker v1.2.0] Precache partial error:', err);
      });
    })
  );
      ...
      return Promise.all(
        cacheNames.map((existingCache) => {
          if (existingCache !== CACHE_NAME) {
            console.info('[ServiceWorker v1.2.0] Cleaning old cache:', existingCache);
